...
class Recognition_Pipeline:
    def __init__(self, recognizer, classifier, idx_to_class):
        print('calling recognizer pipeline')
    ...
    def recognise_faces(self, img):
        bbs, embeddings = self.recognizer.detect_and_extract_feature(img)
        if bbs is None:
